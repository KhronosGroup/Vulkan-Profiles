...
class CombineConfig:
    ...
    def apply_json_value(self, p_name, p_val):
        ...
        if "required-profiles" in p_val:
            req = p_val["required-profiles"]
            if isinstance(req, str):
                self.required_profiles = [p.strip() for p in req.split(',') if p.strip()]
            else:
                self.required_profiles = list(req)
        elif "profiles" in p_val:
            req = p_val["profiles"]
            if isinstance(req, str):
                self.required_profiles = [p.strip() for p in req.split(',') if p.strip()]
            else:
...
class VulkanProfilesCombineGenerator:
    ...
    def combine(self, profile_config, profile_file, mode='intersection'):
        self.mode = mode
        api_ver_str = '.'.join(profile_config.api_version)
        logging.info(f"Building Vulkan {api_ver_str} profile '{profile_config.name}'")
    ...
    def combine_capabilities(self, jsons, profile_names, api_version):
        ...
        for i in range(len(jsons)):
            ...
            # Aggregate all capability blocks for profile i first
            profile_cap = {
                'extensions': {},
                'features': {},
                'properties': {},
